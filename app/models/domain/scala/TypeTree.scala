...
final case class TypeTree(nodes: Seq[Step])
    extends Step with UpdateScopeThrows {

  override def toCompilable: String =
    nodes.map(f => f.toCompilable).mkString(" ")
  ...
  override def fillEmptySteps(
    ...
    fNodesWithoutEmpties.map(
        nodesWithoutEmpties => TypeTree(nodesWithoutEmpties.instructions))
  }
