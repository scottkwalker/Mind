...
final class FactoryLookupImpl @Inject()(injector: Injector)
    extends FactoryLookup {

  override val factories: Set[PozInt] = Set(AddOperatorFactory.id,
                                            FunctionMFactory.id,
                                            IntegerMFactory.id,
                                            TypeTreeFactory.id,
                                            ObjectFactory.id,
  ...
  override def convert(id: PozInt): Decision = id match {
    ...
    case ValDclInFunctionParamFactory.id =>
