...
final class LookupChildrenWithFuturesImpl @Inject()(
  ...
  override def get(scope: IScope,
                   childrenToChooseFrom: Set[PozInt]): Future[Set[Decision]] =
    fetchFromRepository(scope, childrenToChooseFrom).map(
        _.map(factoryLookup.convert))

  private def fetchFromRepository(
      scope: IScope, neighbours: Set[PozInt]): Future[Set[PozInt]] = {
    val neighbourValues = Future.traverse(neighbours) { neighbourId =>
      repository
        .apply(key1 = scope, key2 = neighbourId)
        . // Boolean value from repository.
