...
final class RepositoryReturningBool @Inject()(factoryLookup: FactoryLookup)
    extends Memoize2Impl[IScope, PozInt, Boolean](factoryLookup.version)(
        writes) {
...
object RepositoryReturningBool {
  ...
  private[memoization] implicit val writes =
    new Writes[Map[String, Either[CountDownLatch, Boolean]]] {
      def writes(
          cache: Map[String, Either[CountDownLatch, Boolean]]): JsValue = {
  ...
  private[memoization] implicit def reads(
      factoryLookup: FactoryLookup): Reads[RepositoryReturningBool] =
