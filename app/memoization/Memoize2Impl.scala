...
abstract class Memoize2Impl[TKey1, TKey2, TOutput](
...
)(implicit cacheFormat: Writes[Map[String, Either[CountDownLatch, TOutput]]])
    extends Memoize2[TKey1, TKey2, TOutput] {

  protected var cache: Map[String, Either[CountDownLatch, TOutput]] =
    Map.empty[String, Either[CountDownLatch, TOutput]]
  ...
  /**
    ...
    * table.
    */
  @tailrec protected final def missing(
      implicit key1: TKey1, key2: TKey2): TOutput = {
    val key = combineKeys
    synchronized {
  ...
  private[this] def combineKeys(implicit key1: TKey1, key2: TKey2) =
    s"$key1|$key2"
}
...
object Memoize2Impl {
  ...
  def read[T](json: JsValue)(implicit fromJson: Reads[T]): T =
    JsonDeserialiser.deserialize(json)
}
